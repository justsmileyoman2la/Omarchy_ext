...
// ---- Russian day/month names. The installed Qt's global QmlQt object has
//      no locale overload for Qt.formatDate/Qt.formatDateTime, and the panel
//      is localized to Russian, so the QML bakes names in as quoted literals
//      before calling those functions; numerals and time stay with Qt.
var RU_DAYS_FULL  = ["Воскресенье", "Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота"]
var RU_DAYS_SHORT = ["Вс", "Пн", "Вт", "Ср", "Чт", "Пт", "Сб"]
var RU_MONTHS_NOM = ["январь", "февраль", "март", "апрель", "май", "июнь", "июль", "август", "сентябрь", "октябрь", "ноябрь", "декабрь"]
var RU_MONTHS_GEN = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "августа", "сентября", "октября", "ноября", "декабря"]
var RU_MONTHS_SHORT = ["янв", "фев", "мар", "апр", "май", "июн", "июл", "авг", "сен", "окт", "ноя", "дек"]

// Rewrites a Qt date format so that day and month names come out Russian,
// without passing a locale to Qt: names are inserted as single-quoted
// literals, every other token (numbers, time, "AP", quoted runs) is copied
// untouched. A "MMMM" after a day-of-month token is rendered in the genitive
// ("14 сентября"); one that stands alone is nominative ("Сентябрь").
function russianFormat(date, rawFormat) {
  var format = String(rawFormat)
  var out = ""
  var i = 0
  var n = format.length
  var sawDayNumber = false
  ...
  while (i < n) {
    ...
    if (c === "'") {
      ...
      while (i < n) {
        out += format.charAt(i)
        if (format.charAt(i) === "'") {
          if (i + 1 < n && format.charAt(i + 1) === "'") { out += format.charAt(i + 1); i += 2; continue }
          i++
          break
        }
        i++
    ...
    if (format.startsWith("dddd", i)) {
      out += "'" + RU_DAYS_FULL[weekday] + "'"
      i += 4
      continue
    }
    if (format.startsWith("ddd", i)) {
      out += "'" + RU_DAYS_SHORT[weekday] + "'"
      i += 3
      continue
    }
    if (format.startsWith("MMMM", i)) {
      out += "'" + (sawDayNumber ? RU_MONTHS_GEN[month] : RU_MONTHS_NOM[month]) + "'"
      i += 4
      continue
    }
    if (format.startsWith("MMM", i)) {
      out += "'" + RU_MONTHS_SHORT[month] + "'"
      i += 3
      continue
    }
    if (c === "d") {

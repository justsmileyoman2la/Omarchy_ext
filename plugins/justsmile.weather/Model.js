...
// Human-readable weather condition in Russian, from the same code a color
// would come from, so the pill text stays in sync with the icon.
function currentCondition(current, fallback) {
  if (!current) return fallback || ""
  if (current.openMeteoWeatherCode !== undefined && current.openMeteoWeatherCode !== null)
    return conditionForOpenMeteoCode(current.openMeteoWeatherCode, Number(current.isDay) === 0)
  if (current.weatherCode !== undefined && current.weatherCode !== null)
    return conditionForCode(current.weatherCode, false)
  return fallback || ""
}

function conditionForOpenMeteoCode(code, night) {
  var c = parseInt(String(code || "0"), 10)
  if (c === 0) return night ? "Ясно" : "Солнечно"
  if (c === 1 || c === 2) return "Малооблачно"
  if (c === 3) return "Облачно"
  if (c === 45 || c === 48) return "Туман"
  if (c === 51 || c === 53 || c === 55 || c === 56 || c === 57 || c === 61) return "Морось"
  if (c === 63 || c === 65 || c === 66 || c === 67 || c === 80 || c === 81 || c === 82) return "Дождь"
  if (c === 71 || c === 73 || c === 75 || c === 77 || c === 85 || c === 86) return "Снег"
  if (c === 95 || c === 96 || c === 99) return "Гроза"
  return "Облачно"
}

function conditionForCode(code, night) {
  var c = parseInt(String(code || "0"), 10)
  switch (c) {
    case 113: return night ? "Ясно" : "Солнечно"
    case 116: return "Малооблачно"
    case 143: return "Туман"
    case 119: case 122: return "Облачно"
    case 176: case 263: case 293: case 353: return "Морось"
    case 200: case 386: case 389: case 392: case 395: return "Гроза"
    case 266: case 296: case 299: case 302: case 305: case 308: case 356: case 359: return "Дождь"
    case 179: case 182: case 185: case 227: case 230: case 281: case 284:
    case 311: case 314: case 317: case 320: case 323: case 326: case 329:
    case 332: case 335: case 338: case 350: case 362: case 365: case 368:
    case 371: case 374: case 377: return "Снег"
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    conditionForOpenMeteoCode: conditionForOpenMeteoCode,
